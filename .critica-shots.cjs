...
async function shootRoute(browser, routeKey, hash, vp, theme, action) {
  ...
  consoleErrors[key] = errs.filter((e) => !e.includes('ERR_CONNECTION_RESET') && !e.includes('fonts.googleapis'));

  const fname = `${routeKey}__${vp.name}__${theme}.png`;
  await page.screenshot({ path: path.join(OUT, fname), fullPage: true });
  console.log('saved', fname, 'scrollWidth=', scrollWidth, 'errs=', consoleErrors[key].length);
...
async function main() {
  ...
  fs.writeFileSync(path.join(OUT, '_console-errors.json'), JSON.stringify(consoleErrors, null, 2));
  fs.writeFileSync(path.join(OUT, '_hscroll.json'), JSON.stringify(hScroll, null, 2));
}
